...
const getPredictedCount = async (name: string) => {
  const res = await fetch(`https://api.genderize.io?name=${name}`);
  return res.json();
};
...
async function Prediction({ params }: Params) {
  ...
  const countData = getPredictedCount(params.name)
  const [age, gender, nationality, count] = await Promise.all([
    ageData,
    genderData,
    nationalityData,
    countData
